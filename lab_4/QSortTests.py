...
class QsortTests(unittest.TestCase):
    ...
    def test_empty(self):
        self.assertTrue(BookArray([]) == BookArray([]).qsort_pages())

    def test_matching(self):
        self.assertTrue(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([1, 2, 3])),
            BookArray(pages_list_to_book_list([3, 1, 2])).qsort_pages()))
        self.assertTrue(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([1, 1, 1])),
            BookArray(pages_list_to_book_list([1, 1, 1])).qsort_pages()))
        self.assertTrue(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([2, 5, 5])),
            BookArray(pages_list_to_book_list([5, 2, 5])).qsort_pages()))

    def test_mismatching(self):
        self.assertFalse(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([1, 2, 3])),
            BookArray(pages_list_to_book_list([3, 1, 3])).qsort_pages()))
        self.assertFalse(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([1, 2, 3])),
            BookArray(pages_list_to_book_list([3, 1, 2, 4])).qsort_pages()))
        self.assertFalse(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([1, 2, 3])),
            BookArray(pages_list_to_book_list([])).qsort_pages()))
        self.assertFalse(are_bookArrays_eq_by_pages(
            BookArray(pages_list_to_book_list([5, 2, 5])),
            BookArray(pages_list_to_book_list([2, 5, 2])).qsort_pages()))

    def test_nones(self):
        self.assertTrue(
            BookArray(pages_list_to_book_list([2, 2, 5, None])),
            BookArray(pages_list_to_book_list([2, None, 5, 2])).qsort_pages()
        )
